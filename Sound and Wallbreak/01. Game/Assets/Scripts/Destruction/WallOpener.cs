...
public class WallOpener : MonoBehaviour, INotify
{
    ...
    void OnBulletHit(Notification n)
    {
        ...
        if (go == gameObject)
        {
            Debug.Log(gameObject.name + " (wall) has been hit, will deal with health if necessary");
            currentHealth -= Random.RandomRange(15, 20);
            if (currentHealth < 20)
            {
                Debug.Log(gameObject.name + " is almost dead, killing collider");
